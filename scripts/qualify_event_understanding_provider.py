...
def _provider_model(provider: str) -> str:
    ...
    if provider == "fireworks_kimi_k2p5":
        return FIREWORKS_KIMI_K2P5
...
def _provider_configured(provider: str) -> bool:
    ...
    if provider == "fireworks_kimi_k2p5":
        return FireworksKimiK2P5StructuredClient.configured()
    raise ValueError(f"unsupported qualification provider: {provider}")
...
def _provider_client(provider: str):
    ...
    if provider == "fireworks_kimi_k2p5":
        return FireworksKimiK2P5StructuredClient.from_env(), FIREWORKS_KIMI_K2P5
    raise ValueError(f"unsupported qualification provider: {provider}")
