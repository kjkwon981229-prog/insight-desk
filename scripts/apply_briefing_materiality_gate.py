...
def _briefing_materiality_passes(title_text: str, topic: Topic, event_type: str) -> bool:
    """Keep factual event validity separate from daily briefing importance.

    Strong event families remain untouched.  Only broad announcement/schedule/
    completed-entertainment families need an additional test that the headline
    changes the reader's state of knowledge rather than merely exposing promo,
    lifestyle, interview, or routine calendar content.
    """
    ...
    if family == "kpop":
        ...
        material = any(pattern.search(normalize_text(title_text)) for pattern in _KPOP_BRIEFING_MATERIAL_PATTERNS)
        return material
    ...
    if family == "economy":
        low_information = low_information or any(
            _contains(title_text, marker) for marker in _ECONOMY_LOW_INFORMATION_MARKERS
        )
        return bool(
            any(_contains(title_text, marker) for marker in _ECONOMY_MATERIAL_MARKERS)
            and not low_information
        )

    if family == "kbo":
        return bool(
            any(_contains(title_text, marker) for marker in _KBO_MATERIAL_MARKERS)
            and not low_information
        )

    if family == "psat":
        low_information = low_information or any(
            _contains(title_text, marker) for marker in _PSAT_LOW_INFORMATION_MARKERS
        )
        return bool(
            any(_contains(title_text, marker) for marker in _PSAT_MATERIAL_MARKERS)
            and not low_information
        )

    # Unknown fixture/custom topics keep the previous contract unless the
    # headline is plainly low-information content.
...
class BriefingMaterialityTests(unittest.TestCase):
    ...
    def test_soft_promo_and_lifestyle_content_is_not_briefing_material(self) -> None:
        cases = (
            (
                "kpop-photo", "kpop", "K-POP",
                "아이브, 새 앨범 콘셉트 포토 공개",
                "아이브가 8월 21일 새 앨범 콘셉트 포토를 공식 SNS에 공개했다.",
            ),
            (
                "kpop-playlist", "kpop", "K-POP",
                "BTS, 여름 플레이리스트 공개",
                "BTS 멤버가 8월 21일 직접 고른 여름 플레이리스트를 공개했다.",
            ),
            (
                "kpop-fansign", "kpop", "K-POP",
                "그룹 아이브, 팬사인회 성황",
                "그룹 아이브가 8월 21일 팬사인회를 진행했고 행사를 마쳤다.",
            ),
            (
                "ai-interview", "ai_tech", "OpenAI",
                "OpenAI CEO AI 인터뷰 영상 공개",
                "OpenAI CEO의 AI 산업 인터뷰 영상이 8월 21일 공개됐다.",
            ),
            (
                "kbo-schedule", "kbo_hanwha", "KBO",
                "KBO, 8월 21일 한화-LG 경기 일정 공개",
                "KBO가 8월 21일 한화와 LG의 정규시즌 경기 일정을 공개했다.",
            ),
            (
                "psat-study", "psat_recruitment", "PSAT",
                "PSAT 합격 공부법 영상 공개",
                "PSAT 합격 공부법을 설명하는 영상이 8월 21일 공개됐다.",
            ),
        )
    ...
    def test_material_soft_events_survive(self) -> None:
        cases = (
            (
                "kpop-comeback", "kpop", "K-POP",
                "권은비, 9월 3일 컴백 확정",
                "권은비가 9월 3일 새 디지털 싱글을 발매하며 컴백한다고 발표했다.",
            ),
            (
                "kpop-next-month", "kpop", "K-POP",
                "권은비, 1년 4개월 공백 깨고 내달 컴백",
                "권은비가 내달 새 디지털 싱글을 발매하며 1년 4개월 만에 컴백한다.",
            ),
            (
                "ai-model", "ai_tech", "OpenAI",
                "OpenAI 새 AI 모델 공개",
                "OpenAI가 8월 21일 새 AI 모델을 공개하고 서비스 적용 계획을 발표했다.",
            ),
        )
        for key, topic_id, query, title, summary in cases:
            assessment = self.assessment(key, topic_id, query, title, summary)
    ...
    def test_strong_event_families_remain_eligible(self) -> None:
        cases = (
            (
                "economy-policy", "economy", "금융당국",
                "금융당국 레버리지 ETF 투자한도 100만원 규제 시행",
                "금융당국이 레버리지 ETF 투자한도를 100만원으로 제한하는 규제를 8월 21일부터 시행한다고 발표했다.",
            ),
            (
                "kbo-result", "kbo_hanwha", "한화 이글스",
                "한화 이글스 5-3 승리, 노시환 2홈런",
                "한화 이글스가 5-3으로 승리했고 노시환이 2홈런을 기록했다.",
            ),
            (
                "psat-competition", "psat_recruitment", "7급 공채",
