...
"""One-shot qualification wrapper for Cohere Command A Reasoning under Event Understanding V5."""
...
CANDIDATE_PROVIDER = "cohere_command_a_reasoning"
...
def registered_candidate_provider() -> Iterator[None]:
    ...
    def provider_model(provider: str) -> str:
        if provider == CANDIDATE_PROVIDER:
            return COHERE_COMMAND_A_REASONING
        return original_model(provider)

    def provider_configured(provider: str) -> bool:
        if provider == CANDIDATE_PROVIDER:
            return CohereCommandAReasoningStructuredClient.configured()
        return original_configured(provider)

    def provider_client(provider: str):
        if provider == CANDIDATE_PROVIDER:
            client = CohereCommandAReasoningStructuredClient.from_env()
            return client, COHERE_COMMAND_A_REASONING
