...
"""One-shot qualification wrapper for OpenRouter LFM2.5-2.6B under Event Understanding V5."""
...
CANDIDATE_PROVIDER = "openrouter_lfm25_26b"
...
def registered_candidate_provider() -> Iterator[None]:
    ...
    def provider_model(provider: str) -> str:
        if provider == CANDIDATE_PROVIDER:
            return OPENROUTER_LFM25_26B_FREE
        return original_model(provider)

    def provider_configured(provider: str) -> bool:
        if provider == CANDIDATE_PROVIDER:
            return OpenRouterLFM2526BStructuredClient.configured()
        return original_configured(provider)

    def provider_client(provider: str):
        if provider == CANDIDATE_PROVIDER:
            client = OpenRouterLFM2526BStructuredClient.from_env()
            return client, OPENROUTER_LFM25_26B_FREE
