...
"""One-shot qualification wrapper for Mistral Small 4 under Event Understanding V5.

The candidate is registered only inside this wrapper's context. Historical runners and the
...
CANDIDATE_PROVIDER = "mistral_small4"
...
def registered_candidate_provider() -> Iterator[None]:
    ...
    def provider_model(provider: str) -> str:
        if provider == CANDIDATE_PROVIDER:
            return MISTRAL_SMALL_4
        return original_model(provider)

    def provider_configured(provider: str) -> bool:
        if provider == CANDIDATE_PROVIDER:
            return MistralSmall4StructuredClient.configured()
        return original_configured(provider)

    def provider_client(provider: str):
        if provider == CANDIDATE_PROVIDER:
            client = MistralSmall4StructuredClient.from_env()
            return client, MISTRAL_SMALL_4
