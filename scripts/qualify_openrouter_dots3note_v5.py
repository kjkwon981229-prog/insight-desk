...
"""One-shot qualification wrapper for OpenRouter Dots3-Note under Event Understanding V5."""
...
CANDIDATE_PROVIDER = "openrouter_dots3note"
...
def registered_candidate_provider() -> Iterator[None]:
    ...
    def provider_model(provider: str) -> str:
        if provider == CANDIDATE_PROVIDER:
            return OPENROUTER_DOTS3_NOTE_FREE
        return original_model(provider)

    def provider_configured(provider: str) -> bool:
        if provider == CANDIDATE_PROVIDER:
            return OpenRouterDots3NoteStructuredClient.configured()
        return original_configured(provider)

    def provider_client(provider: str):
        if provider == CANDIDATE_PROVIDER:
            client = OpenRouterDots3NoteStructuredClient.from_env()
            return client, OPENROUTER_DOTS3_NOTE_FREE
