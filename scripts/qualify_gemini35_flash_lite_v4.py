...
"""One-shot qualification wrapper for Gemini 3.5 Flash-Lite under active Event Understanding V4.
...
CANDIDATE_PROVIDER = "gemini35_flash_lite"
...
def registered_candidate_provider() -> Iterator[None]:
    ...
    def provider_model(provider: str) -> str:
        if provider == CANDIDATE_PROVIDER:
            return GEMINI_35_FLASH_LITE
        return original_model(provider)

    def provider_configured(provider: str) -> bool:
        if provider == CANDIDATE_PROVIDER:
            return Gemini35FlashLiteStructuredClient.configured()
        return original_configured(provider)

    def provider_client(provider: str):
        if provider == CANDIDATE_PROVIDER:
            client = Gemini35FlashLiteStructuredClient.from_env()
            return client, GEMINI_35_FLASH_LITE
