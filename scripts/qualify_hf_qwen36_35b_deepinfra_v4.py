...
"""One-shot qualification wrapper for HF Qwen3.6 35B/DeepInfra under active Event Understanding V4.

The candidate is registered only inside this wrapper's context. The canonical V4 runner and the
historical provider registries remain provider-neutral outside that scope.
"""
...
CANDIDATE_PROVIDER = "hf_qwen36_35b_deepinfra"
...
def registered_candidate_provider() -> Iterator[None]:
    ...
    def provider_model(provider: str) -> str:
        if provider == CANDIDATE_PROVIDER:
            return HF_QWEN36_35B_DEEPINFRA
        return original_model(provider)

    def provider_configured(provider: str) -> bool:
        if provider == CANDIDATE_PROVIDER:
            return HuggingFaceQwen36_35BDeepInfraStructuredClient.configured()
        return original_configured(provider)

    def provider_client(provider: str):
        if provider == CANDIDATE_PROVIDER:
            client = HuggingFaceQwen36_35BDeepInfraStructuredClient.from_env()
            return client, HF_QWEN36_35B_DEEPINFRA
