...
"""One-shot qualification wrapper for Gemini 3.6 Flash under active Event Understanding V4.
...
from insight_desk.providers.gemini36 import GEMINI_36_FLASH, Gemini36FlashStructuredClient
from scripts import qualify_event_understanding_provider_v4 as canonical


CANDIDATE_PROVIDER = "gemini36_flash"
...
def registered_candidate_provider() -> Iterator[None]:
    ...
    def provider_model(provider: str) -> str:
        if provider == CANDIDATE_PROVIDER:
            return GEMINI_36_FLASH
        return original_model(provider)

    def provider_configured(provider: str) -> bool:
        if provider == CANDIDATE_PROVIDER:
            return Gemini36FlashStructuredClient.configured()
        return original_configured(provider)

    def provider_client(provider: str):
        if provider == CANDIDATE_PROVIDER:
            client = Gemini36FlashStructuredClient.from_env()
            return client, GEMINI_36_FLASH
