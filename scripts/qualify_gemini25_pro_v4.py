...
"""One-shot qualification wrapper for Gemini 2.5 Pro under active Event Understanding V4.
...
from insight_desk.providers.gemini25pro import GEMINI_25_PRO, Gemini25ProStructuredClient
from scripts import qualify_event_understanding_provider_v4 as canonical


CANDIDATE_PROVIDER = "gemini25_pro"
...
def registered_candidate_provider() -> Iterator[None]:
    ...
    def provider_configured(provider: str) -> bool:
        if provider == CANDIDATE_PROVIDER:
            return Gemini25ProStructuredClient.configured()
        return original_configured(provider)

    def provider_client(provider: str):
        if provider == CANDIDATE_PROVIDER:
            client = Gemini25ProStructuredClient.from_env()
            return client, GEMINI_25_PRO
