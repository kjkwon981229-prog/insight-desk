...
"""One-shot qualification wrapper for OpenRouter Nex-N2-Pro under Event Understanding V5."""
...
CANDIDATE_PROVIDER = "openrouter_nexn2pro"
...
def registered_candidate_provider() -> Iterator[None]:
    ...
    def provider_model(provider: str) -> str:
        if provider == CANDIDATE_PROVIDER:
            return OPENROUTER_NEX_N2_PRO_FREE
        return original_model(provider)

    def provider_configured(provider: str) -> bool:
        if provider == CANDIDATE_PROVIDER:
            return OpenRouterNexN2ProStructuredClient.configured()
        return original_configured(provider)

    def provider_client(provider: str):
        if provider == CANDIDATE_PROVIDER:
            client = OpenRouterNexN2ProStructuredClient.from_env()
            return client, OPENROUTER_NEX_N2_PRO_FREE
