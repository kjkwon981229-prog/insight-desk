...
"""One-shot qualification wrapper for Gemini 3.5 Flash under active Event Understanding V4.
...
from insight_desk.providers.gemini35 import GEMINI_35_FLASH, Gemini35FlashStructuredClient
from scripts import qualify_event_understanding_provider_v4 as canonical


CANDIDATE_PROVIDER = "gemini35_flash"
...
def registered_candidate_provider() -> Iterator[None]:
    ...
    def provider_configured(provider: str) -> bool:
        if provider == CANDIDATE_PROVIDER:
            return Gemini35FlashStructuredClient.configured()
        return original_configured(provider)

    def provider_client(provider: str):
        if provider == CANDIDATE_PROVIDER:
            client = Gemini35FlashStructuredClient.from_env()
            return client, GEMINI_35_FLASH
