...
def deduplicate_news(items: tuple[NewsItem, ...]) -> tuple[NewsItem, ...]:
    ...
    for item in sorted(items, key=_published, reverse=True):
        ...
        if duplicate_index is not None:
            ...
            # A merge can preserve either side's canonical URL/title/hash.
            # Index every alias so a later duplicate can still resolve to the
            # same representative instead of starting a second item.
            for value in (existing.canonical_url, item.canonical_url, merged.canonical_url):
                if value:
                    by_url[value] = duplicate_index
            for value in (existing.content_hash, item.content_hash, merged.content_hash):
                if value:
                    by_hash[value] = duplicate_index
            for value in (existing.title, item.title, merged.title):
                if value:
                    by_title[value] = duplicate_index
            continue
        index = len(kept)
        kept.append(item)
