    ...
    for item in items:
        ...
        for topic_id in topic_ids:
            ...
            for existing_topic_id, cluster in clusters:
                # Clusters are event identities, not connected components.
                # Requiring the new item to agree with every existing member
                # prevents a shared bridge headline from merging two distinct
                # events transitively.
                if existing_topic_id == topic_id and all(_similar(item, member) for member in cluster):
                    cluster.append(item)
                    placed = True
                    break
            if not placed:
