...
"""Qualification-only OpenRouter structured client for Event Understanding.

This provider is intentionally not exported from ``insight_desk.providers`` and is not wired into
production. The exact free-model slug is frozen so qualification cannot silently become random
model routing or a paid fallback path.
"""
...
OPENROUTER_NEMOTRON_3_SUPER_FREE = "nvidia/nemotron-3-super-120b-a12b:free"


@dataclass(slots=True)
class OpenRouterNemotronStructuredClient:
    api_key: str
    model_id: str = OPENROUTER_NEMOTRON_3_SUPER_FREE
    transport: JsonHttpTransport | None = None

    def __post_init__(self) -> None:
        if self.model_id != OPENROUTER_NEMOTRON_3_SUPER_FREE:
            raise ValueError(
                "OpenRouter Event Understanding candidate is frozen to "
