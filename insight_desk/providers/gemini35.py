...
"""Qualification-only Gemini 3.5 Flash client for Event Understanding V4.
...
GEMINI_35_INTERACTIONS_URL = "https://generativelanguage.googleapis.com/v1beta/interactions"


@dataclass(slots=True)
class Gemini35FlashStructuredClient:
    api_key: str
    model_id: str = GEMINI_35_FLASH
    transport: JsonHttpTransport | None = None

    def __post_init__(self) -> None:
        if self.model_id != GEMINI_35_FLASH:
            raise ValueError("Gemini Event Understanding candidate is frozen to gemini-3.5-flash")
    ...
    def _extract_json(response: dict[str, Any]) -> dict[str, object]:
        ...
        if not isinstance(steps, list) or not steps:
            raise ProviderTransportError(
                ...
                detail="Gemini 3.5 interaction has no steps",
            )
        text_parts: list[str] = []
        for step in steps:
        ...
        if not text:
            raise ProviderTransportError(
                failure_kind=FailureKind.INVALID_OUTPUT,
                detail="Gemini 3.5 interaction has no model text",
            )
        try:
            ...
            raise ProviderTransportError(
                failure_kind=FailureKind.INVALID_OUTPUT,
                detail="Gemini 3.5 model text is not valid JSON",
            ) from exc
        if not isinstance(decoded, dict):
            raise ProviderTransportError(
                failure_kind=FailureKind.INVALID_OUTPUT,
                detail="Gemini 3.5 structured output root is not an object",
            )
        return decoded
