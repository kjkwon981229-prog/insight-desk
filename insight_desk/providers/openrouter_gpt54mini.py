...
"""Qualification-only OpenRouter GPT-5.4 Mini structured client for Event Understanding.

This provider is intentionally not exported from ``insight_desk.providers`` and is not wired into
production. The exact model slug is frozen so qualification cannot silently move to an alias,
random model routing, or a different paid fallback model.
"""
...
OPENROUTER_GPT_54_MINI = "openai/gpt-5.4-mini"


@dataclass(slots=True)
class OpenRouterGpt54MiniStructuredClient:
    api_key: str
    model_id: str = OPENROUTER_GPT_54_MINI
    transport: JsonHttpTransport | None = None

    def __post_init__(self) -> None:
        if self.model_id != OPENROUTER_GPT_54_MINI:
            raise ValueError(
                "OpenRouter GPT-5.4 Mini Event Understanding candidate is frozen to "
                "openai/gpt-5.4-mini"
            )
