...
from insight_desk.providers.gemini25pro import (
    GEMINI_25_PRO,
    Gemini25ProStructuredClient,
)
...
from scripts import qualify_gemini25_pro_v4 as lane
...
class Gemini25ProV4QualificationTests(unittest.TestCase):
    def test_candidate_is_exact_new_model_and_production_owner_is_unchanged(self) -> None:
        self.assertEqual(GEMINI_25_PRO, "gemini-2.5-pro")
        self.assertEqual(GEMINI_FLASH_LITE, "gemini-3.1-flash-lite")
        self.assertEqual(lane.CANDIDATE_PROVIDER, "gemini25_pro")
        ...
        with self.assertRaisesRegex(ValueError, "frozen"):
            Gemini25ProStructuredClient(api_key="test", model_id=GEMINI_FLASH_LITE)

    def test_credential_reuses_gemini_secret_without_fallback(self) -> None:
        self.assertFalse(Gemini25ProStructuredClient.configured({}))
        self.assertFalse(Gemini25ProStructuredClient.configured({"GEMINI_API_KEY": "  "}))
        self.assertTrue(Gemini25ProStructuredClient.configured({"GEMINI_API_KEY": "key"}))
        with self.assertRaisesRegex(ProviderConfigError, "GEMINI_API_KEY"):
            Gemini25ProStructuredClient.from_env(env={})
    ...
    def test_structured_json_is_schema_bound_and_makes_one_transport_call(self) -> None:
        ...
        client = Gemini25ProStructuredClient(api_key="test", transport=transport)
        schema = {
            "type": "object",
        ...
        self.assertEqual(payload["model"], GEMINI_25_PRO)
    ...
    def test_invalid_provider_shape_maps_to_bounded_transport_failure(self) -> None:
        client = Gemini25ProStructuredClient(api_key="test", transport=_FakeTransport({"steps": []}))
    ...
    def test_candidate_registration_is_scoped_to_v4_runner_only(self) -> None:
        ...
        with patch.dict(os.environ, {}, clear=True):
            with lane.registered_candidate_provider():
                ...
                self.assertEqual(v4._provider_model(lane.CANDIDATE_PROVIDER), GEMINI_25_PRO)
    ...
    def test_missing_credential_reuses_active_v4_not_configured_contract(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            self.assertEqual(report["model"], GEMINI_25_PRO)
