...
from insight_desk.providers.openrouter_dots3note import (
    OPENROUTER_CHAT_URL,
    OPENROUTER_DOTS3_NOTE_FREE,
    OpenRouterDots3NoteStructuredClient,
)
...
from scripts import qualify_openrouter_dots3note_v5 as lane
...
class OpenRouterDots3NoteV5QualificationTests(unittest.TestCase):
    def test_candidate_is_exact_new_route_not_frozen_openrouter_models(self) -> None:
        self.assertEqual(OPENROUTER_DOTS3_NOTE_FREE, "dots-studio/dots-3-note-preview:free")
        self.assertNotEqual(OPENROUTER_DOTS3_NOTE_FREE, OPENROUTER_NEMOTRON_3_SUPER_FREE)
        self.assertNotEqual(OPENROUTER_DOTS3_NOTE_FREE, OPENROUTER_GLM_52_FREE)
        self.assertEqual(lane.CANDIDATE_PROVIDER, "openrouter_dots3note")
        ...
        with self.assertRaisesRegex(ValueError, "frozen"):
            OpenRouterDots3NoteStructuredClient(
                api_key="test",
                model_id=OPENROUTER_GLM_52_FREE,
            )

    def test_credential_reuses_openrouter_secret_without_fallback(self) -> None:
        self.assertFalse(OpenRouterDots3NoteStructuredClient.configured({}))
        self.assertFalse(
            OpenRouterDots3NoteStructuredClient.configured({"OPENROUTER_API_KEY": "  "})
        )
        self.assertTrue(
            OpenRouterDots3NoteStructuredClient.configured({"OPENROUTER_API_KEY": "key"})
        )
        with self.assertRaisesRegex(ProviderConfigError, "OPENROUTER_API_KEY"):
            OpenRouterDots3NoteStructuredClient.from_env(env={})

    def test_default_transport_disables_hidden_retry(self) -> None:
        client = OpenRouterDots3NoteStructuredClient(api_key="test")
    ...
    def test_structured_json_uses_exact_route_and_strict_schema(self) -> None:
        ...
        client = OpenRouterDots3NoteStructuredClient(api_key="test", transport=transport)
        schema = {
            "type": "object",
        ...
        self.assertEqual(payload["model"], OPENROUTER_DOTS3_NOTE_FREE)
        self.assertEqual(
            payload["messages"],
            [
    ...
    def test_missing_or_invalid_text_maps_to_bounded_transport_failure(self) -> None:
        for response in (
            ...
            with self.subTest(response=response):
                client = OpenRouterDots3NoteStructuredClient(
                    api_key="test",
    ...
    def test_missing_credential_reuses_active_v5_not_configured_contract(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            self.assertEqual(report["model"], OPENROUTER_DOTS3_NOTE_FREE)
