...
HAS_PROPERTY_QA = (
    ...
    and importlib.util.find_spec("kiwipiepy") is not None
)
...
if HAS_PROPERTY_QA:
    ...
    def _article(body: str, index: int = 0) -> RawArticle:
        return RawArticle(
            article_id=f"property-{index}",
            provenance=SourceProvenance(
                source_id="fixture:property",
            ...
            title="property source preservation",
            body=body,
            topic_ids=("ai_tech",),
        )
    ...
    class SemanticPropertyTests(unittest.TestCase):
        ...
        @given(
            st.lists(st.sampled_from(SENTENCES), min_size=1, max_size=5),
            st.sampled_from((" ", "\n", "\n\n", "  ")),
        )
        def test_kiwi_sentence_spans_always_roundtrip_to_exact_source(
            self,
            sentences: list[str],
            separator: str,
        ) -> None:
        ...
        def test_emitted_fact_fields_never_escape_cited_source(
            self,
            subject: str,
            object_text: str,
            action: str,
