...
from scripts import qualify_mistral_medium35_v5 as lane
...
class MistralMedium35V5QualificationTests(unittest.TestCase):
    def test_candidate_is_exact_new_model_and_historical_route_is_unchanged(self) -> None:
        self.assertEqual(MISTRAL_MEDIUM_35, "mistral-medium-3-5")
        self.assertEqual(MISTRAL_LARGE_3, "mistral-large-2512")
        self.assertNotEqual(MISTRAL_MEDIUM_35, MISTRAL_LARGE_3)
        self.assertEqual(lane.CANDIDATE_PROVIDER, "mistral_medium35")
        ...
        with self.assertRaisesRegex(ValueError, "frozen"):
            MistralMedium35StructuredClient(api_key="test", model_id=MISTRAL_LARGE_3)

    def test_credential_reuses_mistral_secret_without_fallback(self) -> None:
        self.assertFalse(MistralMedium35StructuredClient.configured({}))
        self.assertFalse(MistralMedium35StructuredClient.configured({"MISTRAL_API_KEY": "  "}))
        self.assertTrue(MistralMedium35StructuredClient.configured({"MISTRAL_API_KEY": "key"}))
        with self.assertRaisesRegex(ProviderConfigError, "MISTRAL_API_KEY"):
            MistralMedium35StructuredClient.from_env(env={})
    ...
    def test_structured_json_is_schema_bound_and_makes_one_transport_call(self) -> None:
        ...
        client = MistralMedium35StructuredClient(api_key="test", transport=transport)
        schema = {
            "type": "object",
        ...
        self.assertEqual(payload["model"], MISTRAL_MEDIUM_35)
        self.assertEqual(
            payload["messages"],
            [
    ...
    def test_invalid_provider_shape_maps_to_bounded_transport_failure(self) -> None:
        client = MistralMedium35StructuredClient(
    ...
    def test_candidate_registration_is_scoped_to_v5_runner_only(self) -> None:
        ...
        with patch.dict(os.environ, {}, clear=True):
            with lane.registered_candidate_provider():
                ...
                self.assertEqual(v5._provider_model(lane.CANDIDATE_PROVIDER), MISTRAL_MEDIUM_35)
    ...
    def test_missing_credential_reuses_active_v5_not_configured_contract(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            self.assertEqual(report["model"], MISTRAL_MEDIUM_35)
