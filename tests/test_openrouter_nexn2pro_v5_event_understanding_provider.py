...
from insight_desk.providers.openrouter_nexn2pro import (
    OPENROUTER_CHAT_URL,
    OPENROUTER_NEX_N2_PRO_FREE,
    OpenRouterNexN2ProStructuredClient,
)
...
from scripts import qualify_openrouter_nexn2pro_v5 as lane
...
class OpenRouterNexN2ProV5QualificationTests(unittest.TestCase):
    def test_candidate_is_exact_new_route_not_frozen_openrouter_models(self) -> None:
        self.assertEqual(OPENROUTER_NEX_N2_PRO_FREE, "nex-agi/nex-n2-pro:free")
        self.assertNotEqual(OPENROUTER_NEX_N2_PRO_FREE, OPENROUTER_NEMOTRON_3_SUPER_FREE)
        self.assertNotEqual(OPENROUTER_NEX_N2_PRO_FREE, OPENROUTER_GLM_52_FREE)
        self.assertNotEqual(OPENROUTER_NEX_N2_PRO_FREE, OPENROUTER_DOTS3_NOTE_FREE)
        self.assertEqual(lane.CANDIDATE_PROVIDER, "openrouter_nexn2pro")
    ...
    def test_credential_reuses_openrouter_secret_without_fallback(self) -> None:
        self.assertFalse(OpenRouterNexN2ProStructuredClient.configured({}))
        self.assertFalse(
            OpenRouterNexN2ProStructuredClient.configured({"OPENROUTER_API_KEY": "  "})
        )
        self.assertTrue(
            OpenRouterNexN2ProStructuredClient.configured({"OPENROUTER_API_KEY": "key"})
        )
        with self.assertRaisesRegex(ProviderConfigError, "OPENROUTER_API_KEY"):
            OpenRouterNexN2ProStructuredClient.from_env(env={})

    def test_default_transport_disables_hidden_retry(self) -> None:
        client = OpenRouterNexN2ProStructuredClient(api_key="test")
    ...
    def test_structured_json_uses_exact_route_and_strict_schema(self) -> None:
        ...
        client = OpenRouterNexN2ProStructuredClient(api_key="test", transport=transport)
        schema = {
            "type": "object",
        ...
        self.assertEqual(payload["model"], OPENROUTER_NEX_N2_PRO_FREE)
        self.assertEqual(
            payload["messages"],
            [
    ...
    def test_missing_or_invalid_text_maps_to_bounded_transport_failure(self) -> None:
        for response in (
            ...
            with self.subTest(response=response):
                client = OpenRouterNexN2ProStructuredClient(
                    api_key="test",
    ...
    def test_missing_credential_reuses_active_v5_not_configured_contract(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            self.assertEqual(report["model"], OPENROUTER_NEX_N2_PRO_FREE)
