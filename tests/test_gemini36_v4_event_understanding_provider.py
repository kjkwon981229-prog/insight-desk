...
from insight_desk.providers.gemini36 import (
    GEMINI_36_FLASH,
    Gemini36FlashStructuredClient,
)
...
from scripts import qualify_gemini36_flash_v4 as lane
...
class Gemini36FlashV4QualificationTests(unittest.TestCase):
    def test_candidate_is_exact_new_model_and_production_owner_is_unchanged(self) -> None:
        self.assertEqual(GEMINI_36_FLASH, "gemini-3.6-flash")
        self.assertEqual(GEMINI_FLASH_LITE, "gemini-3.1-flash-lite")
        self.assertEqual(lane.CANDIDATE_PROVIDER, "gemini36_flash")
        ...
        with self.assertRaisesRegex(ValueError, "frozen"):
            Gemini36FlashStructuredClient(api_key="test", model_id=GEMINI_FLASH_LITE)

    def test_credential_reuses_gemini_secret_without_fallback(self) -> None:
        self.assertFalse(Gemini36FlashStructuredClient.configured({}))
        self.assertFalse(Gemini36FlashStructuredClient.configured({"GEMINI_API_KEY": "  "}))
        self.assertTrue(Gemini36FlashStructuredClient.configured({"GEMINI_API_KEY": "key"}))
        with self.assertRaisesRegex(ProviderConfigError, "GEMINI_API_KEY"):
            Gemini36FlashStructuredClient.from_env(env={})
    ...
    def test_structured_json_is_schema_bound_and_makes_one_transport_call(self) -> None:
        ...
        client = Gemini36FlashStructuredClient(api_key="secret", transport=transport)
        schema = {
            "type": "object",
        ...
        self.assertEqual(payload["model"], GEMINI_36_FLASH)
    ...
    def test_invalid_provider_shape_maps_to_bounded_transport_failure(self) -> None:
        client = Gemini36FlashStructuredClient(
    ...
    def test_candidate_registration_is_scoped_to_v4_runner_only(self) -> None:
        ...
        with patch.dict(os.environ, {}, clear=True):
            with lane.registered_candidate_provider():
                ...
                self.assertEqual(v4._provider_model(lane.CANDIDATE_PROVIDER), GEMINI_36_FLASH)
    ...
    def test_missing_credential_reuses_active_v4_not_configured_contract(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            self.assertEqual(report["model"], GEMINI_36_FLASH)
