...
class Phase11ProductionRestoreTests(unittest.TestCase):
    ...
    def test_workflow_restores_schedule_pages_and_push_without_legacy_engine(self) -> None:
        ...
        self.assertIn("grep -q '오늘 알아둘 뉴스를 모았습니다.'", workflow)
        self.assertIn("Internal verification copy leaked into the user-facing briefing.", workflow)
        self.assertNotIn("grep -q 'SUPPORTED headline + summary only'", workflow)
