...
class Live224RetrospectiveStrategyRegressions(unittest.TestCase):
    ...
    def test_live_undated_strategy_background_is_context_dependent_material(self) -> None:
    ...
    def test_full_named_undated_strategy_background_remains_non_event_analytical(self) -> None:
        headline = "문혁수 사장, ‘고수익 사업 포트폴리오’ 핵심으로 반도체 기판 지목"
        summary = (
        ...
        values = _issue_values(topic="AI·테크", headline=headline, summary=summary)
        self.assertIn("FEED_QUALITY_NON_EVENT_ANALYTICAL_SUMMARY", values)
        self.assertNotIn("FEED_QUALITY_CONTEXT_DEPENDENT_HEADLINE", values)
