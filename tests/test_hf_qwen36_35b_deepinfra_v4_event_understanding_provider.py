...
from insight_desk.providers.hf_qwen36_35b_deepinfra import (
    HF_QWEN36_35B_DEEPINFRA,
    HF_ROUTER_CHAT_URL,
...
from scripts import qualify_hf_qwen36_35b_deepinfra_v4 as lane
...
class HFQwen36_35BDeepInfraV4QualificationTests(unittest.TestCase):
    def test_candidate_is_exact_new_model_and_not_a_production_export(self) -> None:
        self.assertEqual(HF_QWEN36_35B_DEEPINFRA, "Qwen/Qwen3.6-35B-A3B:deepinfra")
        self.assertEqual(HF_QWEN3_235B_2507_NSCALE, "Qwen/Qwen3-235B-A22B-Instruct-2507:nscale")
        self.assertNotEqual(HF_QWEN36_35B_DEEPINFRA, HF_QWEN3_235B_2507_NSCALE)
        self.assertEqual(lane.CANDIDATE_PROVIDER, "hf_qwen36_35b_deepinfra")
        self.assertFalse(hasattr(production_providers, "HuggingFaceQwen36_35BDeepInfraStructuredClient"))
        ...
        with self.assertRaisesRegex(ValueError, "frozen"):
            HuggingFaceQwen36_35BDeepInfraStructuredClient(
                api_key="test",
                model_id=HF_QWEN3_235B_2507_NSCALE,
            )

    def test_default_transport_disables_hidden_http_retry(self) -> None:
        client = HuggingFaceQwen36_35BDeepInfraStructuredClient(api_key="test")
        self.assertIsNotNone(client.transport)
        self.assertEqual(client.transport.attempts, 1)

    def test_credential_reuses_hf_secret_without_fallback(self) -> None:
        self.assertFalse(HuggingFaceQwen36_35BDeepInfraStructuredClient.configured({}))
        self.assertFalse(HuggingFaceQwen36_35BDeepInfraStructuredClient.configured({"HF_TOKEN": "  "}))
        self.assertTrue(HuggingFaceQwen36_35BDeepInfraStructuredClient.configured({"HF_TOKEN": "key"}))
        with self.assertRaisesRegex(ProviderConfigError, "HF_TOKEN"):
            HuggingFaceQwen36_35BDeepInfraStructuredClient.from_env(env={})

    def test_structured_json_uses_exact_hf_schema_route_and_one_transport_call(self) -> None:
        ...
        client = HuggingFaceQwen36_35BDeepInfraStructuredClient(api_key="test", transport=transport)
        schema = {
            "type": "object",
        ...
        self.assertEqual(payload["model"], HF_QWEN36_35B_DEEPINFRA)
        self.assertEqual(
            payload["messages"],
            [
    ...
    def test_invalid_provider_shape_maps_to_bounded_transport_failure(self) -> None:
        client = HuggingFaceQwen36_35BDeepInfraStructuredClient(api_key="test", transport=_FakeTransport({}))
    ...
    def test_candidate_registration_is_scoped_to_v4_runner_only(self) -> None:
        ...
        with patch.dict(os.environ, {}, clear=True):
            with lane.registered_candidate_provider():
                ...
                self.assertEqual(v4._provider_model(lane.CANDIDATE_PROVIDER), HF_QWEN36_35B_DEEPINFRA)
    ...
    def test_missing_credential_reuses_active_v4_not_configured_contract(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            self.assertEqual(report["model"], HF_QWEN36_35B_DEEPINFRA)
