...
class Phase5Phase6OwnerWiringTests(unittest.TestCase):
    def test_production_runtime_scopes_phase6_to_v2_owner_and_restores_it(self) -> None:
        original = production._core.Phase6EventEngine
        with production_v2_runtime(production._core):
            self.assertIs(
                production._core.Phase6EventEngine,
    ...
    def test_v2_phase6_does_not_call_legacy_semantic_material_classifier(self) -> None:
        ...
        try:
            result = EvidenceIntegrityPhase6EventEngine().assess_with_auto_material(
                event,
                facts={fact_id: fact},
