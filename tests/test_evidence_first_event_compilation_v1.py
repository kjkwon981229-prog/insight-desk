...
class EvidenceFirstEventCompilationContracts(unittest.TestCase):
    def test_background_status_claim_stays_non_primary_without_analytical_blacklist(self) -> None:
        ...
        self.assertIs(compilation.status, UnderstandingStatus.RESOLVED)
        self.assertEqual(compilation.primary_claim_id, "claim:event:0")
        self.assertIs(
            compilation.assignment("claim:event:0").article_role,
            ArticleEventRole.PRIMARY,
        )
        self.assertIs(
            compilation.assignment("claim:event:1").article_role,
            ArticleEventRole.SUPPORTING,
        )

    def test_later_lineup_claim_does_not_beat_lead_event(self) -> None:
    ...
    def test_psat_claim_preserves_exact_change_semantics_without_introduction_rewrite(self) -> None:
        first = (
        ...
        action = (
            "내년부터 별도 검정시험 형태로 시행돼 국가공무원 5·7급 공채 등의 "
            "1차 시험을 대체한다"
        )
        article = _article(
        ...
        self.assertEqual(draft.action, action)
        self.assertNotIn("도입", draft.action)
        self.assertEqual(draft.actor, "공직적격성평가(PSAT)")
        self.assertEqual(draft.object, "국가공무원 5·7급 공채 등의 1차 시험")

    def test_canonical_boundary_accepts_only_selected_primary_claim(self) -> None:
        ...
        result = _result(
            ...
            [
                (
                    "앤트로픽",
                    "로봇 제어용 하드웨어 표준을 공개했다",
                    "로봇 제어용 하드웨어 표준",
                    first,
                ),
                (
                    "에이전트",
                    "실제 로봇 등 물리적 장비를 안전하게 제어할 수 있다",
                    "실제 로봇 등 물리적 장비",
                    second,
                ),
            ],
        ...
        self.assertIsNotNone(draft)
        assert draft is not None
        self.assertEqual(draft.actor, "앤트로픽")
        self.assertTrue(all(ref.source_id == source.source_id for ref in draft.evidence_refs))
    ...
    def test_deep_claim_without_lead_or_title_binding_abstains(self) -> None:
        ...
        self.assertIs(compilation.status, UnderstandingStatus.UNRESOLVED)
        self.assertIsNone(compilation.primary_claim_id)
        self.assertIsNone(canonical_draft_from_primary_claim(compilation))
        self.assertIn("article_centrality_unresolved", compilation.uncertainty_reasons)

    def test_incomplete_predicate_is_explicit_unresolved_claim_not_drop(self) -> None:
        sentence = "지원하는 기업 계획."
        ...
        self.assertIs(claims[0].completeness, ClaimCompleteness.UNRESOLVED)
        self.assertEqual(claims[0].uncertainty_reasons, ("predicate_incomplete",))
        self.assertIs(compilation.status, UnderstandingStatus.UNRESOLVED)
        self.assertIsNone(compilation.primary_claim_id)

    def test_nonliteral_semantic_field_is_rejected_before_claim_contract(self) -> None:
        ...
        with self.assertRaisesRegex(ContractError, "not literal"):
            compile_evidence_bound_claims(
                article,
                result,
                morphology=_FiniteMorphology(),
