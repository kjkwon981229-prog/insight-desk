...
class PipelineTests(unittest.TestCase):
    ...
    def test_clustering_does_not_use_transitive_single_link_bridge(self) -> None:
        first = NewsItem(
            "bridge-first", "ai", "AI", "삼성전자 AI 규제 발표", "규제안이 발표됐다.",
            "https://bridge-first.test", "", "https://bridge-first.test", None,
            "bridge-first.test", "bridge-first",
        )
        bridge = NewsItem(
            "bridge-middle", "ai", "AI", "삼성전자 규제 세부안 공개 출시", "세부안이 공개됐다.",
            "https://bridge-middle.test", "", "https://bridge-middle.test", None,
            "bridge-middle.test", "bridge-middle",
        )
        third = NewsItem(
            "bridge-third", "ai", "AI", "삼성전자 신제품 출시", "신제품 출시 일정이 공개됐다.",
            "https://bridge-third.test", "", "https://bridge-third.test", None,
            "bridge-third.test", "bridge-third",
        )
        clusters = cluster_news((first, bridge, third))
        self.assertEqual(sorted(len(cluster.items) for cluster in clusters), [1, 2])

    def test_deduplication_reindexes_alternate_keys_after_merge(self) -> None:
        first = NewsItem(
            "alias-first", "t", "q", "첫 번째 제목", "긴 원문 요약", "https://a.test/one", "",
            "https://a.test/one", None, "a.test", "hash-a",
        )
        second = NewsItem(
            "alias-second", "t", "q", "두 번째 제목", "짧은 요약", "https://b.test/two", "",
            "https://b.test/two", None, "b.test", "hash-a",
        )
        third = NewsItem(
            "alias-third", "t", "q", "두 번째 제목", "새로운 후속 요약", "https://b.test/two", "",
            "https://b.test/two", None, "b.test", "hash-c",
        )
        result = deduplicate_news((first, second, third))
        self.assertEqual(len(result), 1)
        self.assertEqual(result[0].summary, "새로운 후속 요약")
